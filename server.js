...
// 1. Listado de tools disponibles
app.get('/mcp', (req, res) => {
  res.json({
    name: 'vtex-mcp',
    version: '1.0.0',
    description: 'MCP Server para VTEX — órdenes, productos e inventario',
    tools: [
      {
        name: 'get_orders',
      ...
      {
        name: 'get_products',
        description: 'Obtiene productos del catálogo VTEX con su stock e información',
        inputSchema: {
          type: 'object',
          properties: {
...
app.post('/mcp/call', async (req, res) => {
  ...
  try {
    ...
    if (tool === 'get_orders') {
      ...
      result = await vtexGet(`/api/catalog_system/pvt/products/GetProductAndSkuIds?_from=${from}&_to=${to}${cat}`);
    }

    else if (tool === 'get_inventory') {
      result = await vtexGet(`/api/logistics/pvt/inventory/skus/${input.skuId}`);
    }
